...
const Footer = () => {
  const footerLinks = {
    ...
    'Categories': [
      { name: 'Luxury Watches', href: '/categories/watches' },
  ...
  return (
    <footer className="bg-luxury-black border-t border-dark-gray">
      {/* Trust Features */}
            ...
            <div className="lg:col-span-2">
              ...
              >
                <Link href="/" className="inline-block mb-6">
                  <h2 className="text-3xl font-montserrat font-black text-luxury-gold tracking-wider">
                    FLYFE
                  </h2>
                </Link>
                
                <p className="text-gray-300 font-poppins leading-relaxed mb-6 max-w-sm">
                  Redefining luxury e-commerce with curated collections of premium products 
                ...
                <div className="flex gap-4">
                  {socialLinks.map((social) => (
                    <motion.a
                      ...
                      whileTap={{ scale: 0.95 }}
                      className="w-10 h-10 bg-dark-gray hover:bg-luxury-gold hover:text-luxury-black text-gray-300 rounded-full flex items-center justify-center transition-all duration-300"
            ...
            {Object.entries(footerLinks).map(([category, links], categoryIndex) => (
              <div key={category}>
                <motion.div
                  ...
                  viewport={{ once: true }}
                >
                  <h3 className="text-luxury-gold font-poppins font-semibold mb-6 text-lg">
                    {category}
                  </h3>
                  <ul className="space-y-3">
                    {links.map((link, linkIndex) => (
                      ...
                      >
                        <Link
                          href={link.href}
                          className="text-gray-300 hover:text-luxury-gold transition-colors duration-300 font-poppins text-sm block py-1"
                        >
                          {link.name}
                        </Link>
                      </motion.li>
      ...
      <div className="border-t border-dark-gray py-8">
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            >
              <Link href="/privacy" className="hover:text-luxury-gold transition-colors duration-300">
                Privacy Policy
              </Link>
              <Link href="/terms" className="hover:text-luxury-gold transition-colors duration-300">
                Terms of Service
              </Link>
              <Link href="/cookies" className="hover:text-luxury-gold transition-colors duration-300">
                Cookie Policy
              </Link>
            </motion.div>
